...
export default function Home() {
    ...
    const [showLoader, setShowLoader] = useState(true);     // Loader visibility
    const [showContent, setShowContent] = useState(false);  // Main content mount
    ...
    useEffect(() => {
        const handlePageLoad = () => {
            ...
            setTimeout(() => {
                setShowLoader(false);
                setShowContent(true); // Now show the page content
            }, 1000); // matches loader fade duration
        };
    ...
    return (
        <>
            {/* Loader */}
            {showLoader && (
                <div className="fixed inset-0 z-[9999] bg-black flex items-center justify-center transition-opacity duration-1000 opacity-100">
                    <p className="text-white text-xl animate-pulse">Loading...</p>
                </div>
            )}

            {/* Main Content (Only renders after loader fully hidden) */}
            {showContent && (
                <div
                    className={`relative flex w-screen h-screen items-center justify-center bg-cover bg-center transition-opacity duration-1000 opacity-100`}
